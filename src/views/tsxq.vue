<template>
  <div class="main">
    ...
    <div class="conten"><span>作者:{{zz}}</span><span style="margin-left:8rem">主播:{{zb}}</span>
    <div><span>集数:{{js}}</span><span style="margin-left:8rem">每集价格:{{jg}}阅点</span>
    
    </div>
    <div style="margin-left:1rem">书籍简介: <br>
    {{nr}}
    </div>
...
export default {
  ...
  methods: {
    ...
    getbook() {
      ...
      get(`https://apis.netstart.cn/yunyuedu/ting/catalog.json?sourceUuid=${id}`).then(res=>{
        console.log(res);
      })
      get('https://apis.netstart.cn/yunyuedu/ting/download.json').then(res=>{
